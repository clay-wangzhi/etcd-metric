...
import (
	...
	_ "etcd-operator/pkg/clusterprovider/providers" // register cluster provider
	"etcd-operator/pkg/controllers/util"
	"etcd-operator/pkg/etcd"
	"etcd-operator/pkg/featureprovider"

	_ "etcd-operator/pkg/featureprovider/providers" // register feature provider
	...
	informers "etcd-operator/pkg/generated/informers/externalversions/etcd/v1alpha1"
	listers "etcd-operator/pkg/generated/listers/etcd/v1alpha1"
)
...
func (c *ClusterController) updateEtcdClusterStatus(cluster *etcdv1alpha1.EtcdCluster) (
	*etcdv1alpha1.EtcdCluster,
	...
	etcdcluster, err := c.platformclientset.EtcdV1alpha1().EtcdClusters(cluster.Namespace).
...
func (c *ClusterController) handleClusterManagement(cluster *etcdv1alpha1.EtcdCluster) (
	*etcdv1alpha1.EtcdCluster,
	error,
) {
	...
	switch nextAction {
	case etcdv1alpha1.EtcdCluterCreating:
		cluster, err = c.handleClusterCreate(cluster, provider)
	case etcdv1alpha1.EtcdClusterUpdating:
...
func (c *ClusterController) enabledFeatureGate(annotations map[string]string, name etcdv1alpha1.KStoneFeature) bool {
...
func (c *ClusterController) handleClusterLabels(
	cluster *etcdv1alpha1.EtcdCluster) (*etcdv1alpha1.EtcdCluster, error) {
	...
	for featureName := range featureprovider.EtcdFeatureProviders {
		labels[featureName] = strconv.FormatBool(c.enabledFeatureGate(annotations, etcdv1alpha1.KStoneFeature(featureName)))
	}
...
func (c *ClusterController) handleClusterFeature(cluster *etcdv1alpha1.EtcdCluster) (
	*etcdv1alpha1.EtcdCluster,
	error) {
	// Check cluster status, ensure cluster is running
	if cluster.Status.Phase != etcdv1alpha1.EtcdClusterRunning || len(cluster.Status.Members) == 0 {
	...
	if cluster.Status.FeatureGatesStatus == nil {
		cluster.Status.FeatureGatesStatus = make(map[etcdv1alpha1.KStoneFeature]string)
	}
	for name := range featureprovider.EtcdFeatureProviders {
		featureName := etcdv1alpha1.KStoneFeature(name)
...
func (c *ClusterController) reconcileEtcdCluster(cluster *etcdv1alpha1.EtcdCluster) error {
	...
	if cluster.Status.Phase != etcdv1alpha1.EtcdClusterRunning {
...
func (c *ClusterController) GetEtcdClusterProvider(name etcdv1alpha1.EtcdClusterType) (clusterprovider.Cluster, error) {
...
func (c *ClusterController) getDesiredAction(
...
) (etcdv1alpha1.EtcdClusterPhase, error) {
	if len(cluster.Status.Conditions) == 0 {
		return etcdv1alpha1.EtcdCluterCreating, nil
	}
	...
	case etcdv1alpha1.EtcdClusterConditionCreate:
		if lastCondition.Status == corev1.ConditionFalse {
			return etcdv1alpha1.EtcdCluterCreating, nil
		}
	case etcdv1alpha1.EtcdClusterConditionUpdate:
		if lastCondition.Status == corev1.ConditionFalse {
			return etcdv1alpha1.EtcdClusterUpdating, nil
		}
	}
	...
	if err != nil {
		...
		return etcdv1alpha1.EtcdClusterUnknown, err
	}
	if !equal {
	...
	return etcdv1alpha1.EtcdClusterRunning, nil
}

func (c *ClusterController) generateConditions(
	conditions []etcdv1alpha1.EtcdClusterCondition,
	phase etcdv1alpha1.EtcdClusterPhase,
	nextConditionType etcdv1alpha1.EtcdClusterConditionType,
) []etcdv1alpha1.EtcdClusterCondition {
	...
	conditions = append(conditions, etcdv1alpha1.EtcdClusterCondition{
...
func (c *ClusterController) handleClusterCreate(
	cluster *etcdv1alpha1.EtcdCluster,
	provider clusterprovider.Cluster,
) (*etcdv1alpha1.EtcdCluster, error) {
	cluster.Status.Conditions = c.generateConditions(
		cluster.Status.Conditions,
		cluster.Status.Phase,
		etcdv1alpha1.EtcdClusterConditionCreate,
	)
	cluster.Status.Phase = etcdv1alpha1.EtcdCluterCreating
...
func (c *ClusterController) handleClusterUpdate(
	...
	cluster.Status.Conditions = c.generateConditions(
		cluster.Status.Conditions,
		cluster.Status.Phase,
		etcdv1alpha1.EtcdClusterConditionUpdate,
	)
	cluster.Status.Phase = etcdv1alpha1.EtcdClusterUpdating
	conditionIndex := len(cluster.Status.Conditions) - 1
